#!/usr/bin/env python3
"""Availability monitor for inverter connection/control percentages.

- Pulls current-month availability metrics from API response:
    * dataPointPercentage -> DisponibilitaPripojeni
    * controlPercentage   -> DisponibilitaRizeni
  Values are fractions (0..1).

- Triggers notifications when either metric drops below:
    WARN: 92%
    CRIT: 90%

- Uses a small state file to avoid spamming (notify only on level changes).

Notification channels:
- email (Gmail app password supported)
"""

from __future__ import annotations

import json
import os
import smtplib
import sys
import time
from dataclasses import dataclass
from datetime import datetime
from email.message import EmailMessage
from typing import Any, Dict, Optional, Tuple

import requests
from zoneinfo import ZoneInfo


STATE_PATH = os.environ.get("STATE_PATH", "monitor/state.json")

# Thresholds in percent (0..100)
WARN_THRESHOLD_PCT = float(os.environ.get("WARN_THRESHOLD_PCT", "92"))
CRIT_THRESHOLD_PCT = float(os.environ.get("CRIT_THRESHOLD_PCT", "90"))

# API returns fractions (0..1), so convert thresholds accordingly.
WARN_THRESHOLD = WARN_THRESHOLD_PCT / 100.0
CRIT_THRESHOLD = CRIT_THRESHOLD_PCT / 100.0

PRAGUE_TZ = ZoneInfo(os.environ.get("LOCAL_TZ", "Europe/Prague"))


@dataclass(frozen=True)
class AvailabilitySample:
    dispon_pripojeni: float  # dataPointPercentage (0..1)
    dispon_rizeni: float     # controlPercentage (0..1)
    month_start_utc: datetime
    month_start_local: datetime


def _load_state() -> Dict[str, Any]:
    if os.path.exists(STATE_PATH):
        with open(STATE_PATH, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"last_level": "OK", "last_notified_at": None, "last_month": None}


def _save_state(state: Dict[str, Any]) -> None:
    os.makedirs(os.path.dirname(STATE_PATH), exist_ok=True)
    with open(STATE_PATH, "w", encoding="utf-8") as f:
        json.dump(state, f, indent=2, ensure_ascii=False)


def _iso_z_to_dt(s: str) -> datetime:
    # Example: "2026-02-28T23:00:00.000Z"
    return datetime.fromisoformat(s.replace("Z", "+00:00"))


def _pick_current_month_record(records: list[dict]) -> dict:
    now_local = datetime.now(tz=PRAGUE_TZ)
    target_year, target_month = now_local.year, now_local.month

    best: Optional[dict] = None
    for r in records:
        m = r.get("month")
        if not m:
            continue
        dt_utc = _iso_z_to_dt(m)
        dt_local = dt_utc.astimezone(PRAGUE_TZ)
        if (dt_local.year, dt_local.month) == (target_year, target_month):
            if best is None:
                best = r
            else:
                prev_dt = _iso_z_to_dt(best["month"])
                if dt_utc > prev_dt:
                    best = r

    if best is None:
        latest: Optional[Tuple[datetime, dict]] = None
        for r in records:
            m = r.get("month")
            if not m:
                continue
            dt_utc = _iso_z_to_dt(m)
            dt_local = dt_utc.astimezone(PRAGUE_TZ)
            if (dt_local.year, dt_local.month) <= (target_year, target_month):
                if latest is None or dt_utc > latest[0]:
                    latest = (dt_utc, r)
        if latest is None:
            raise ValueError("No usable 'records' entries with a 'month' field.")
        best = latest[1]

    return best


def fetch_current_month_availability() -> AvailabilitySample:
    """Fetch availability and return current-month metrics.

    The API response (tRPC batch) can be nested; this searches for the first
    object containing a 'records' list.
    """
    url = os.environ["AVAILABILITY_API_URL"]

    headers: Dict[str, str] = {}
    token = os.environ.get("AVAILABILITY_API_TOKEN")
    if token:
        headers["Authorization"] = f"Bearer {token}"

    cookie = os.environ.get("AVAILABILITY_COOKIE")
    if cookie:
        headers["Cookie"] = cookie

    r = requests.get(url, headers=headers, timeout=30)
    r.raise_for_status()
    data = r.json()

    def find_records(obj: Any) -> Optional[list[dict]]:
        if isinstance(obj, dict):
            if "records" in obj and isinstance(obj["records"], list):
                return obj["records"]
            for v in obj.values():
                rec = find_records(v)
                if rec is not None:
                    return rec
        elif isinstance(obj, list):
            for it in obj:
                rec = find_records(it)
                if rec is not None:
                    return rec
        return None

    records = find_records(data)
    if not records:
        raise ValueError("Could not find 'records' list in API response.")

    rec = _pick_current_month_record(records)

    dp = float(rec["dataPointPercentage"])
    cp = float(rec["controlPercentage"])
    month_utc = _iso_z_to_dt(rec["month"])
    month_local = month_utc.astimezone(PRAGUE_TZ)

    return AvailabilitySample(
        dispon_pripojeni=dp,
        dispon_rizeni=cp,
        month_start_utc=month_utc,
        month_start_local=month_local,
    )


def classify(sample: AvailabilitySample) -> str:
    worst = min(sample.dispon_pripojeni, sample.dispon_rizeni)
    if worst < CRIT_THRESHOLD:
        return "CRIT"
    if worst < WARN_THRESHOLD:
        return "WARN"
    return "OK"


def format_subject(level: str, sample: AvailabilitySample) -> str:
    return f"FVE Availability {level} - {sample.month_start_local.strftime('%Y-%m')}"


def format_body(level: str, sample: AvailabilitySample, previous: str) -> str:
    return (
        f"FVE Availability monitor status: {level}\n"
        f"Month (local): {sample.month_start_local.strftime('%Y-%m')} (Europe/Prague)\n"
        f"DisponibilitaPripojeni (dataPointPercentage): {sample.dispon_pripojeni*100:.2f}%\n"
        f"DisponibilitaRizeni (controlPercentage): {sample.dispon_rizeni*100:.2f}%\n"
        f"Previous level: {previous}\n"
        f"Thresholds: WARN<{WARN_THRESHOLD_PCT:.0f}%, CRIT<{CRIT_THRESHOLD_PCT:.0f}%\n"
        f"Time: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC\n"
    )


def _send_email(subject: str, body: str) -> None:
    # Gmail SMTP settings
    smtp_host = os.environ.get("SMTP_HOST", "smtp.gmail.com")
    smtp_port = int(os.environ.get("SMTP_PORT", "587"))

    smtp_user = os.environ["SMTP_USER"]        # full gmail address
    smtp_password = os.environ["SMTP_PASSWORD"]  # app password

    mail_from = os.environ.get("MAIL_FROM", smtp_user)
    mail_to = os.environ["MAIL_TO"]  # comma-separated list allowed

    msg = EmailMessage()
    msg["Subject"] = subject
    msg["From"] = mail_from
    msg["To"] = mail_to
    msg.set_content(body)

    with smtplib.SMTP(smtp_host, smtp_port, timeout=30) as server:
        server.ehlo()
        server.starttls()
        server.ehlo()
        server.login(smtp_user, smtp_password)
        server.send_message(msg)


def notify_email(subject: str, body: str) -> None:
    _send_email(subject, body)


def main() -> int:
    state = _load_state()
    prev_level = state.get("last_level", "OK")

    sample = fetch_current_month_availability()
    level = classify(sample)

    subject = format_subject(level, sample)
    body = format_body(level, sample, prev_level)

    # Notify only when level changes (OK<->WARN<->CRIT)
    if level != prev_level:
        notify_email(subject, body)
        state["last_level"] = level
        state["last_notified_at"] = int(time.time())
        state["last_month"] = sample.month_start_local.strftime("%Y-%m")
        _save_state(state)
    else:
        state["last_month"] = sample.month_start_local.strftime("%Y-%m")
        _save_state(state)

    # Optional: make the workflow fail when WARN/CRIT so it’s obvious in Actions UI.
    if level in ("WARN", "CRIT"):
        return 2
    return 0


if __name__ == "__main__":
    sys.exit(main())
